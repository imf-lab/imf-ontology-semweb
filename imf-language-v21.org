...
The diagram displays the type patterns. The patterns that are directly
instantiated to create types are marked with C, while the abstract
patterns, which are not directly instantiated, are marked with A.
...
sh.NodeShape -right-> sh.PropertyShape : sh.property

abstract class sh.PropertyShape {
  sh.path: 1 iri
}
...
sh.PropertyShape <|-- NodeRef
NodeRef --> "1" sh.NodeShape : node
...
sh.PropertyShape <|-- ComplexValues
...
**** Short description of the type patterns
 ...
 - NodeShape :: specifies a collection Property shapes. Used for
                specifying a BlockType, TerminalType,
 ...
 - NodeRef :: specifies a reference to node shape with
...
The diagram below displays a suggested use of the type patterns. All
the class nodes marked with M indicate Type which are represented as
sh:NodeShape pattern instances. Open arrows represent a specialisation
dependency. Regular arrows represent a suggested use of the NodeRef
pattern. The fields in the class boxes represent uses of other
suggested pattern instances as indicated by the fields.
...
ElementType --> AttributeGroupType : NodeRef
ElementType --> AttributeType : NodeRef
BlockType --> TerminalType : NodeRef
AttributeGroupType --> AttributeType : NodeRef
...
| NodeShape | name     | description      | targetClass |
|-----------+----------+------------------+-------------|
| ex:node1  | 'myname' | 'my description' | ex:MyClass  |
...
o-imf:ShaclPropertyShapeValues[
    ottr:IRI ?nodeshape, 
    ottr:IRI ?path, 
  ? xsd:string ?message, 
...
o-imf:ShaclPropertyShape[
  ...
  ottr:Triple(_:P, sh:in, ?inValues),
  ottr:Triple(_:P, sh:hasValue, ?value),
  ottr:Triple(_:P, sh:minInclusive, ?minInclusive),
  ottr:Triple(_:P, sh:minExclusive, ?minExclusive),
  ottr:Triple(_:P, sh:maxInclusive, ?maxInclusive),
  ottr:Triple(_:P, sh:maxExclusive, ?maxExclusive),
  ottr:Triple(_:P, sh:minLength, ?minLength),
  ottr:Triple(_:P, sh:maxLength, ?maxLength),
  ottr:Triple(_:P, sh:pattern, ?pattern),
  ottr:Triple(_:P, sh:flags, ?flags)
} .
#+END_SRC
...
<<py_ottr_instances(template="o-imf:ShaclPropertyShapeValue", table=shacl-test-property-value)>>
<<py_ottr_instances(template="o-imf:ShaclPropertyShapeValues", table=shacl-test-property-values)>>
<<py_ottr_instances(template="o-imf:ShaclPropertyShapeNode", table=shacl-test-property-node)>>
#+END_SRC
...
[] ottr:of o-imf:ShaclNodeShape ;
   ottr:values ( ex:Node1 'myname' 'my description' ex:MyClass ) . 

[] ottr:of o-imf:ShaclPropertyShape ;
   ottr:values ( ex:node1 ex:p1 'message' sh:Warning 1 3 'value' ( ex:c1 ex:c2 ) ex:node2 ex:MyClass2 xsd:datatype 1 2 3 4 5 6 'pattern' 'flag' ) . 

[] ottr:of o-imf:ShaclPropertyShapeValue ;
   ottr:values ( ex:node1 ex:p1 'value' ) . 

[] ottr:of o-imf:ShaclPropertyShapeValues ;
   ottr:values ( ex:node1 ex:p1 'Error: bla' 1 3 'value' ( ex:c1 ex:c2 ) ex:MyClass2 xsd:datatype ) . 

[] ottr:of o-imf:ShaclPropertyShapeNode ;
   ottr:values ( ex:node1 ex:p1 'Error: bla' 1 3 ex:node2 ) . 
#+end_example
...
#+RESULTS:
#+begin_example
rapper: Parsing URI file:///home/martige/repo/imf-lab/imf-ontology/out/temp/example-shacl-shapes.wottr.ttl.temp with parser turtle
rapper: Serializing with serializer turtle
